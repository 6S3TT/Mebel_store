...
from .models import Products, Basket, FavouriteProduct, ProductImage, Subcategory, Categories
...
def favorite_products(request):
    favourite_products_box = FavouriteProduct.objects.filter(user=request.user)
    subcategories = Subcategory.objects.all()
    sub_1 = subcategories[0:4]
    sub_2 = subcategories[4:7]
    sub_3 = subcategories[6]
    context = {
        ...
        'products': favourite_products_box,
        'subcategories': subcategories,
        'sub_1': sub_1,
        'sub_2': sub_2,
        'sub_3': sub_3
...
def basket_remove(request, basket_id):
    basket_box = Basket.objects.get(id=basket_id)
    basket_box.delete()
    return HttpResponseRedirect(request.META['HTTP_REFERER'])
...
def product_search(request):
    ...
    return render(request, 'main_index.html', {'products': products, 'query': query})
...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            django_messages.error(request, 'Возникли ошибки при заполнении формы!')
    else:
        form = MessageForm()
    subcategories = Subcategory.objects.all()
    sub_1 = subcategories[0:4]
